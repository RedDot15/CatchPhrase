        ...
        for(int round = 1; round <= ServerMain.rounds; round++) {
            ...
            try {
                ...
                if(round<ServerMain.rounds) {
                    if ((ServerMain.scoreList.get(0) == 20 && ServerMain.scoreList.get(1) == 0) ||
                            (ServerMain.scoreList.get(1) == 20 && ServerMain.scoreList.get(0) == 0)){
                        sendResOut(ServerMain.getWinners() + " has reach 2 round win first\n");
                        sendResOut("Winner:\n" + ServerMain.getWinners());
                        sendResOut("GAME OVER");
                        sendResOut("Round: " + round + "   word: " + word);
                        GameDAO.IncreasePoint(ServerMain.getWinners(), 1.0);
                        System.exit(0);
                        break;
                    }
    ...
    public void gamePlay(int pnum,String word,Thread timer){
        ...
        try {
            ...
            while (timer.isAlive()){
                ...
                if (word.equals(guess.toLowerCase()) && !correct){
